            ...
            {navItems.map((item) => {
              ...
              return (
                <Link key={item.path} href={item.path} asChild>
                  <Button variant="ghost" size="sm" className={isActive ? 'bg-accent text-accent-foreground' : ''} data-testid={`link-nav-${item.label.toLowerCase()}`}>
      ...
      <div className="md:hidden border-t border-border">
        <div className="flex items-center justify-around py-2 px-2">
          {navItems.map((item) => {
            ...
            return (
              <Link key={item.path} href={item.path} asChild>
                <Button variant="ghost" size="sm" className={`flex-col h-auto py-2 ${isActive ? 'bg-accent text-accent-foreground' : ''}`} data-testid={`link-mobile-${item.label.toLowerCase()}`}>
