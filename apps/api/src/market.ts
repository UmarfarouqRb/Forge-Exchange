...
// Store last generated synthetic price offset for each pair to ensure continuity
const syntheticPriceOffsets: { [pairId: string]: number } = {};
const SYNTHETIC_UPDATE_INTERVAL = 3000; // 3-5 seconds, setting to 3s for now
let lastSyntheticUpdate: { [pairId: string]: number } = {}; // Track last update time per pair
let syntheticDepthCache: { [pairId: string]: OrderBook } = {}; // Cache for synthetic depth per pair

function generateSyntheticDepth(midPrice: number, pairId: string): OrderBook {
    const now = Date.now();
    // Update the offset periodically if the interval has passed for this pair
    if (!lastSyntheticUpdate[pairId] || (now - lastSyntheticUpdate[pairId] > SYNTHETIC_UPDATE_INTERVAL)) {
        syntheticPriceOffsets[pairId] = (Math.random() * 0.002 - 0.001); // -0.1% to +0.1%
    ...
    for (let i = 1; i <= 10; i++) {
        const priceBid = fluctuatingMidPrice * (1 - i * 0.002 - (Math.random() * 0.0001)); // Slight random variation
        const quantityBid = (Math.random() * 15 + 5) * (1 + (Math.random() * 0.05 - 0.025)); // Vary quantity by +/- 2.5%
        bids.push([priceBid.toFixed(4), quantityBid.toFixed(2)]);

        const priceAsk = fluctuatingMidPrice * (1 + i * 0.002 + (Math.random() * 0.0001)); // Slight random variation
        const quantityAsk = (Math.random() * 15 + 5) * (1 + (Math.random() * 0.05 - 0.025)); // Vary quantity by +/- 2.5%
...
async function getOrderBook(baseToken: Token, quoteToken: Token, pairId: string): Promise<OrderBook & { lastTradePrice: number | null }> {
    ...
    if (realOrders.length > 0) {
        ...
        if (midPrice) {
            const now = Date.now();
            // Check if synthetic depth needs to be regenerated for this specific pair
            if (!lastSyntheticUpdate[pairId] || (now - lastSyntheticUpdate[pairId] > SYNTHETIC_UPDATE_INTERVAL)) {
                const newSyntheticDepth = generateSyntheticDepth(midPrice, pairId);
                syntheticDepthCache[pairId] = newSyntheticDepth;
                lastSyntheticUpdate[pairId] = now;
            }
            // Use the cached synthetic depth for the current pair
...
export async function getMarket(pairId: string): Promise < MarketState | null > {
    const now = Date.now();
    // Do not cache synthetic order books if they are updated frequently, or allow caching if 'mock' indicates synthetic behavior.
    // The previous condition 'cache[pairId].data.source !== 'mock'' was preventing synthetic data from being cached,
    // but the issue was 'realOrders' not being defined here.
    // Now, we will simply cache all market states generated by getOrderBook, as getOrderBook itself handles
    // the dynamic updating of synthetic data and the fallback to cached synthetic data.
    ...
    try {
        ...
        // Cache all marketState objects.
        // The dynamic nature of synthetic data is handled within getOrderBook.
