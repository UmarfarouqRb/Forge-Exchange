...
import { saveOrder, type InsertOrder } from '@forge/db';
import { getPairById } from './pairs';

export async function createOrder(order: Omit<InsertOrder, 'id' | 'createdAt' | 'status'>) {
    // Basic validation
    if (!order.pairId || !order.userAddress || !order.side || !order.quantity || !order.price) {
        throw new Error('Missing required order fields');
    }

    const pair = await getPairById(order.pairId);
    if (!pair) {
        throw new Error(`Invalid pairId: ${order.pairId}`);
    }
